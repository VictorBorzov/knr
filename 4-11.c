...
int getop(char s[]) {
	...
	static int pos = 0; /* next free position in buffer */

	int i, c, cn;

	while ((s[0] = c = (pos > 0) ? buffer[--pos] : getchar()) == ' ' || c == '\t' || c == '\n');
	s[1] = '\0';

	/* read sign */
	i = 0;
	if (c == '-' || c == '+') {
		if (isdigit(cn = (pos > 0) ? buffer[--pos] : getchar())) {
			s[i] = c;
			c = cn;
		}
		buffer[pos++] = cn;
	...
	/* not a number */
	if (!isdigit(c) && c != '.') {
		cn = (pos > 0) ? buffer[--pos] : getchar();
		if (cn == EOF || cn == '\n' || cn == ' ' || cn == '\t' || isdigit(cn)) {
			/* printf("DEBUG: one char c=%c,cn=%c\n", c, cn); */
			buffer[pos++] = cn;
			// variable
			if (c >= 'a' && c <= 'z') {
				s[0] = c;
			...
			/* printf("DEBUG: command found!, c=%c,cn=%c\n", c, cn); */
			s[i++] = c;
			s[i++] = cn;
			while ((c = (pos > 0) ? buffer[--pos] : getchar()) != EOF && c != '\n' && c != ' ' && c != '\t') {
				s[i++] = c;
			}
			s[i] = '\0';
			return COMMAND;
	...
	if (isdigit(c)) /* collect integer part */
		while (isdigit(s[++i] = c = (pos > 0) ? buffer[--pos] : getchar()));
