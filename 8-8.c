...
void *mmalloc(size_t nbytes) {
	...
	if (nbytes == 0 || nbytes > SSIZE_MAX - nunits * sizeof(Header)) {
		fprintf(stderr, "ERROR: mmalloc: cannot allocate %d bytes\n", nbytes);
		return NULL;
	}
...
/* bfree: add an arbitrary block p of n size into the free list */
void bfree(void *p, size_t n) {
	Header *h;

	/* need one header + non-zero number of bytes */
...
void mfree(void *a) {
	...
	if (a == NULL || (a-1) == NULL) {
		fprintf(stderr, "ERROR: mfree: invalid pointer\n");
		return;
	}
	...
	/* checking block size is valid */
	if (bp->s.size <= 0 || bp->s.size > SSIZE_MAX - NALLOC * sizeof(Header)) {
		fprintf(stderr, "ERROR: mfree: unvalid block size %d\n", bp->s.size);
		return;
	}
	...
	freep = p;
	fprintf(stderr, "DEBUG: mfree: new freep header is %p(%d)\n", freep, freep->s.size);
}
